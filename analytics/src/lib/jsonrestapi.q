...
queryparams:{
  split:"?"vs x;
  if[1=count split; :0N];
  .logger.dev "Found url query parameters";
  params:raze 1_split;
...
listen:{[p]
  .z.ph::{
    ...
    .logger.dev "Received GET";
    .logger.dev .j.j getreq;
    f::matchGetResponder getreq.url;
    if[not 0N~f;.logger.dev "Matched endpoint"];
    getres::$[ 0N~f ; jsonResponse "none" ;  f getreq ];
    .logger.dev "Sending response";
    .logger.dev getres;
    getres};
  .z.pp::{
    rawpostreq::x;
    postreq::.post.request x;
    .logger.dev "Received POST";
    .logger.dev .j.j postreq;
  ...
  .z.pm::{
    ...
    .logger.dev "Received OPTIONS";
    .logger.dev .j.j optreq;
